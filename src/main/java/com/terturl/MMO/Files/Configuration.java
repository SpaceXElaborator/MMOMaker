	...
	public Configuration() {
		...
		if(!configurationFile.exists()) {
			try {
				...
				Block b = Bukkit.getWorlds().get(0).getHighestBlockAt(new Location(Bukkit.getWorlds().get(0), 9.5, 124, 300.5));
		...
		if(config.contains("classNpc")) {
			for(String s : config.getConfigurationSection("classNpc").getKeys(false)) {
				if(MinecraftMMO.getInstance().getClassHandler().containsClass(s)) {
					Location loc = LocationUtils.locationDeSerializer(config.getConfigurationSection("classNpc").getString(s));
