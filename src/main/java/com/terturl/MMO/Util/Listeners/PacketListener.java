...
/**
 * Handles Net.Minecraft.SRC packets and translates them into readable packets
 * for ingestion into the plugin
 * 
 * @author Sean Rahman
 * @since 0.25.0
 *
...
public class PacketListener {
	...
	/**
	 * Get the players ChannelPipeline and add the Custom information below
	 * 
	...
	public static void injectPlayer(Player p) {
		ChannelDuplexHandler handler = new ChannelDuplexHandler() {
			...
			public void channelRead(ChannelHandlerContext context, Object packet) throws Exception {
				// MinecraftMMO.getInstance().getLogger().log(Level.INFO, "PACKET READ: " +
				// packet.toString());
				if (packet instanceof PacketPlayInUseEntity) {
					try {
						...
						if (test.getClass() == testClass) {
							...
							// This will only send 1 for either a or b, but what is both? They seem to both
							// work when right clicking
							if (hand == EnumHand.b) {

								// Because all of these are called in sync, I have to asyncronously call the
								// events
								if (MinecraftMMO.getInstance().getNpcHandler().isNpc(id)) {
									Future<ClickNPCEvent> temp = Bukkit.getScheduler()
											.callSyncMethod(MinecraftMMO.getInstance(), new Callable<ClickNPCEvent>() {
												@Override
												public ClickNPCEvent call() throws Exception {
													ClickNPCEvent e = new ClickNPCEvent(
															MinecraftMMO.getInstance().getNpcHandler().getNPC(id), p,
															bool);
													return e;
												}
											});
									...
									Future<ClickClassNPCEvent> temp = Bukkit.getScheduler().callSyncMethod(
											MinecraftMMO.getInstance(), new Callable<ClickClassNPCEvent>() {
												@Override
												public ClickClassNPCEvent call() throws Exception {
													ClickClassNPCEvent e = new ClickClassNPCEvent(
															MinecraftMMO.getInstance().getNpcHandler().getClassNPC(id),
															p, bool);
													return e;
												}
											});
									...
									Future<ClickPlayerClassNPCEvent> temp = Bukkit.getScheduler().callSyncMethod(
											MinecraftMMO.getInstance(), new Callable<ClickPlayerClassNPCEvent>() {
												@Override
												public ClickPlayerClassNPCEvent call() throws Exception {
													ClickPlayerClassNPCEvent e = new ClickPlayerClassNPCEvent(
															MinecraftMMO.getInstance().getPlayerHandler()
																	.getPlayerClassNPC(p, id),
															p, bool);
													return e;
												}
											});
			...
			public void write(ChannelHandlerContext ctx, Object packet, ChannelPromise promise) throws Exception {
				// MinecraftMMO.getInstance().getLogger().log(Level.INFO, "PACKET WRITE: " +
				// packet.toString());
	...
	/**
	 * Remove the custom pipeline from the player
	 * 
	 * @param p Player to de-inject
	 */
